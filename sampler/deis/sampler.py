...
from .vpsde import get_linear_alpha_fns
...
def get_sampler(sde,alphas_cump, r, eps_fn, ts_phase, ts_order, num_step, method="rho_rk",ab_order=2, rk_method="3kutta"):
    if method.lower() == "rho_rk":
        ...
        return get_sampler_t_ab(sde, alphas_cump, r, eps_fn, ts_phase, ts_order, num_step, ab_order)
    elif method.lower() == "ipndm":
        return get_sampler_ipndm(sde, eps_fn, num_step)
    raise RuntimeError(f"{method} not support!!")

def get_sampler_t_ab(sde, alphas_cump, r, eps_fn, ts_phase, ts_order, num_step, ab_order):
    ...
    # ours
    # t2alpha_fn, alpha2t_fn = get_linear_alpha_fns(0.0001,0.02)
    # jax_alpha_array = t2alpha_fn(jax_rev_ts)
    # th_alpha_array = jax2th(jax_alpha_array)
    alphas_cump = alphas_cump[list(map(int,th_rev_ts))]


    def sampler(xT, y, cut_off_value=None,window_size=None,shift_time_step=False):
        rev_ts, ab_coef = th_rev_ts.to(xT.device), th_ab_coef.to(xT.device)
        x0_prev = None
        def ab_body_fn(i, val, cut_off_value=cut_off_value, window_size=window_size,shift_time_step=shift_time_step):
            x, eps_pred, x0_prev = val
            s_t= rev_ts[i]
            new_eps = eps_fn(x, s_t, y, cut_off_value=cut_off_value, window_size=window_size, shift_time_step=shift_time_step)
            # ours
            # alpha_t = th_alpha_array[i]
            # alpha_next = th_alpha_array[i+1]
            alpha_t = alphas_cump[i]
            alpha_next = alphas_cump[i+1]

            new_x, new_eps_pred, x0_prev = ab_step(x, ab_coef[i], new_eps, eps_pred, x0_prev,alpha_t,alpha_next,r)
            return new_x, new_eps_pred, x0_prev


        eps_pred = [xT,] * ab_order
        img, _ , _= fori_loop(0, num_step, ab_body_fn, (xT, eps_pred, x0_prev))
        return img
    return sampler
...
def get_sampler_ipndm(sde, eps_fn, num_step):
    ...
    def sampler(xT, cut_off_value=None, window_size=None, shift_time_step=None):
        rev_ts, ab_coef = th_rev_ts.to(xT.device), th_ab_coef.to(xT.device)
        def ab_body_fn(i, val):
            ...
            new_eps = eps_fn(x, s_t,
              ...
              shift_time_step=shift_time_step)
