...
class DPMSolverSampler(object):
    ...
    def sample(self,
        ...
        dpm_solver = DPM_Solver(model_fn, self.noise_schedule,  predict_x0=False, 
                                ...
                                apply_time_shift=self.shift_time_step,r=self.r)
        x = dpm_solver.sample(img, steps=S, skip_type="time_uniform", method='multistep', order=2, lower_order_final=True, y=y)
        # x = self.inverse_img_transform(x)
        return th.clip(x,-1,1), y
