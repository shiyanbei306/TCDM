    ...
    def __init__(
        ...
        ***
        ...
        if schedule == 'discrete':
            ...
            self.alpha_list = [1-x for x in alphas_cump]
...
def model_wrapper(
    ...
    def noise_pred_fn(x, t_continuous, cond=None):
        ...
        t_cut = get_shared_timestep(split_points, 100, t_input)
        # t_cut = t_input
        if cond is None:
            output = model(x, t_cut, **model_kwargs)
        else:
            output = model(x, t_cut, cond, **model_kwargs)
        if model_type == "noise":
            return output
    ...
    def model_fn(x, t_continuous, y):
        ...
        if guidance_type == "uncond":
            ...
            et = noise_pred_fn(x, t_continuous, cond=y)
            if omega > 0:
                unc_et = noise_pred_fn(x, t_continuous, cond=None)
                guide = et - unc_et
                et = et + omega * guide
            return et
...
class DPM_Solver:
    def __init__(self, model_fn, noise_schedule, apply_time_shift=False, window_shift=None, cut_off_value=None, predict_x0=False, thresholding=False, max_val=1.,r=0.0):
    ...
    def time_shift(self, img_list, t_next):
        x_pre = img_list[-1]
        n = x_pre.shape[0]
        ### t is in 0-1 map it back to 0-1000
        t_next = ((t_next - 1./self.noise_schedule.total_N)*1000).int()
        t_next = t_next[0]
        var = torch.var(x_pre.view(x_pre.size()[0],-1),dim=-1)
        var.reshape(-1,1)
        if t_next - self.window_shift > 0 and t_next+self.window_shift+1<len(self.noise_schedule.alpha_list):
            time_list = self.noise_schedule.alpha_list[t_next-self.window_shift:t_next+self.window_shift+1]
        elif t_next-self.window_shift <= 0:
            time_list = self.noise_schedule.alpha_list[0:t_next+self.window_shift+1]
        elif t_next+self.window_shift+1 >= len(self.alpha_bar):
            time_list = self.noise_schedule.alpha_list[t_next-self.window_shift:]

        time_list = torch.tensor([time_list]*var.size()[0])
        var = var.unsqueeze(1).expand_as(time_list)
        
        dist = (var - time_list.to(x_pre.device)) ** 2
        next_t = torch.argmin(dist,dim=1)
        ...
        if t_next > self.cut_off_value:
            next_t = torch.tensor(n_t).to(x_pre.device)
        else:
            next_t = (torch.ones(n).to(x_pre.device) * (t_next)).to(x_pre.device)
        torch.cuda.empty_cache()
        next_t = (next_t / 1000 + 1/self.noise_schedule.total_N)
        return next_t
  

    def noise_prediction_fn(self, x, t, y):
        """
    ...
    def model_fn(self, x, t, y):
        ...
        if self.predict_x0:
            ...
            return self.noise_prediction_fn(x, t, y)
    ...
    def multistep_dpm_solver_second_update(self, x, model_prev_list, t_prev_list, t, solver_type="dpm_solver"):
        ...
        alpha_t = torch.exp(log_alpha_t)
        alpha_0 = torch.exp(log_alpha_prev_0)
        ...
        if self.predict_x0:
            ...
            if solver_type == 'dpm_solver':
                ...
                # )
                x_0 = x*1/ expand_dims(alpha_0,dims) - expand_dims(sigma_prev_0/alpha_0,dims)*model_prev_0
                self.x_0_l.append(x_0)
                if len(self.x_0_l) > 1 and self.r != 0.0:
                    x_0 = (1+self.r)*x_0-self.r*self.x_0_l[-2]
                x_t = expand_dims(alpha_t,dims)*x_0 + expand_dims(torch.sqrt(1 - torch.pow(alpha_t,2)),dims)*model_prev_0 - 0.5 * expand_dims(sigma_t * (torch.exp(h) - 1.), dims) * D1_0
            elif solver_type == 'taylor':
                x_t = (
                    expand_dims(torch.exp(log_alpha_t - log_alpha_prev_0), dims) * x
                    - expand_dims(sigma_t * (torch.exp(h) - 1.), dims) * model_prev_0
    ...
    def sample(self, x, steps=20, t_start=None, t_end=None, order=3, skip_type='time_uniform',
        ...
        if method == 'adaptive':
            ...
            assert steps >= order
            timesteps = self.get_time_steps(skip_type=skip_type, t_T=t_T, t_0=t_0, N=steps, device=device) # uniform
            assert timesteps.shape[0] - 1 == steps

            with torch.no_grad():
                self.x_0_l = []
                vec_t = timesteps[0].expand((x.shape[0]))
                model_prev_list = [self.model_fn(x, vec_t, y)]
                t_prev_list = [vec_t]
                ...
                for init_order in range(1, order):
                    ...
                    model_prev_list.append(self.model_fn(x, vec_t,y))
                    t_prev_list.append(vec_t)
                # Compute the remaining values by `order`-th order multistep DPM-Solver.
                for step in tqdm(range(order, steps + 1)):
                    if not self.apply_time_shift:
                        vec_t = timesteps[step].expand(x.shape[0])
                    else:
                        vec_t = self.time_shift(model_prev_list, timesteps[step].expand(x.shape[0]))
                    ...
                    if step < steps:
                        model_prev_list[-1] = self.model_fn(x, vec_t,y)
